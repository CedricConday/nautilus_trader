...
#[rstest]
#[case::aggregate(CatalogType::Data(NautilusDataType::Instrument), "Instrument")]
#[case::class(CatalogType::Instrument(NautilusInstrumentType::Equity), "Equity")]
#[case::record(CatalogType::Record(NautilusRecordType::AccountState), "AccountState")]
...
#[rstest]
fn test_extend_file_name_resolves_the_class_holding_the_identifier() {
    let temp_dir = TempDir::new().unwrap();
    let catalog = ParquetDataCatalog::new(temp_dir.path(), None, None, None, None);
    ...
    equity.ts_event = UnixNanos::from(2_000);
    equity.ts_init = UnixNanos::from(2_000);
    let equity_id = Instrument::id(&equity).to_string();
    catalog
        .write_instruments(vec![InstrumentAny::Equity(equity)])
        .unwrap();
    ...
    assert_eq!(
        catalog
            .get_intervals(&NautilusInstrumentType::Equity.into(), Some(&equity_id))
            .unwrap(),
        vec![(2_000, 3_000)]
    );
